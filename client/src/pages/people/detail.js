...
function PeopleDetail() {
    ...
    function rawDataTochartData(encodedParentName, block) {
        return new Promise((resolve, reject) => {
            const data = [];
            fetch(`http://localhost:9000/api/people/${encodedParentName}/rankingIndex/${block}`)
    ...
    async function fetchData() {
        try {
        const response = await fetch(`http://localhost:9000/api/people/${encodedParentName}`);
...
function PeopleDetail() {
    ...
    function generateChartOption(block) {
        return new Promise((resolve, reject) => {
            console.log("generateChartOption")
            // 查询blockName分类下的所有数据
            const data = []; // 格式为 [{ year: '2015', ranking_index: 100 }, { year: '2016', ranking_index: 200 }, ...]
                ...
                .then(result => {
                    ...
                    const option = {
                      ...
                      yAxis: {
                        ...
                        min: data.length > 0 ? null : 0, // 只有当数据不为空时，最小值为 null，否则为 0
                        max: data.length > 0 ? null : 1000, // 只有当数据不为空时，最大值为 null，否则为 1000
                        splitNumber: data.length > 0 ? null : 5, // 只有当数据不为空时，分割数为 null，否则为 10
                      },
                        series: [
                            {
                                name: parentPresent.toString(),
                                type: 'line',
                                data: chartData.map(item => item.rankingIndex),
                            },
                        ],
                    ...
                    //         fill: '#666',
                    //     },
                    //     invisible: data.length > 0, // 控制是否显示图形元素
                    // },
                    };
    ...
    function updateChartOption(block) {
        return new Promise((resolve, reject) => {
            console.log("updateChartOption:" + parentName1)
            // 查询blockName分类下的所有数据
            const data = []; // 格式为 [{ year: '2015', ranking_index: 100 }, { year: '2016', ranking_index: 200 }, ...]
            const encodedParentName1 = encodeURIComponent(parentName1);
            // 发送查询请求
            fetch(`http://localhost:9000/api/people/${encodedParentName1}/rankingIndex/${block}`)
                .then(response => response.json())
                .then(result => {
                    // 将查询结果格式化成 ECharts 所需的数据格式
                    result.forEach(item => {
                    ...
                    setChartOption(chartOption => {
                        const legendData = [...chartOption.legend.data, parentPresent1.toString()];
                        const seriesData = [
                            ...chartOption.series,
                            {
                                name: parentPresent1.toString(),
                                type: 'line',
                                data: chartData.map(item => item.rankingIndex),
                            },
                        ];
    ...
    function handleUpdateChart() {
        setparentName1('张近东');
        setparentPresent1('张近东');
    }

    useEffect(() => {
        if (parentName1) {
            // 点击按钮更新图表数据
            console.log('parentName changed');
            updateChartOption(currentBlock)
                .then(newOption => setChartOption(newOption))
                .catch((error) => {
                    console.error(error);
                });
        }
    ...
    useEffect(() => {
        ...
        if (currentBlock) {
            generateChartOption(currentBlock).then(option => setChartOption(option));
        }
        if (parentName1) {
            // 更新图表数据
            updateChartOption(currentBlock)
    ...
    <div>
        ...
        <div>
            <div>
                <button onClick={handleUpdateChart}>更新图表</button>
            </div>
            <select value={currentBlock} onChange={(event) => setCurrentBlock(event.target.value)}>
                <option value="all">All</option>
                <option value="财新周刊">财新周刊</option>
                <option value="观察">观察</option>
                <option value="环球时报">环球时报</option>
            ...
            <div id="chart"></div>
            {chartOption && <ReactEcharts ref={chartRef} option={chartOption} />}
        </div>
        <h3>维基百科</h3>
        <p>{wikiLongBio}</p>
        <h3>百度百科</h3>
